...
namespace EW12SG.Function {
    public class LogFile {
        ...
        string dir_log_total = string.Format("{0}Logtotal\\EW12SG", myGlobal.dir_Path);
        string dir_log_detail = string.Format("{0}Logdetail\\EW12SG\\{1}", myGlobal.dir_Path, DateTime.Now.ToString("yyyy-MM-dd"));
        ...
        public bool saveLogDetail() {
            try {
                string f = string.Format("{0}\\EW12SG_{1}_{2}_{3}.txt", dir_log_detail, myGlobal.myTesting.macStamp, DateTime.Now.ToString("hhmmss"), myGlobal.myTesting.totalResult);
                using (var sw = new StreamWriter(f, true, Encoding.Unicode)) {
                    sw.WriteLine("Product: EW12SG\r\n" + myGlobal.myTesting.sshLog);
                }
                return true;
            } catch { return false; }
        }
        ...
        public bool saveLogTotal() {
            try {
                string f = string.Format("{0}\\EW12SG_{1}.csv", dir_log_total, DateTime.Now.ToString("yyyyMMdd"));
